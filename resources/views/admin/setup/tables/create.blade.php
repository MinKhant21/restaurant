  ...
  <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
      ...
      <div class="relative bg-white rounded-lg shadow">
          ...
          <div class="flex items-center p-5 rounded-t border-b border-gray-200 relative">
            <a href="{{ route('admin.tables.index') }}" class="absolute left-0 ml-8 text-sm">
          ...
          <div class="p-6 space-y-6">
              <form action="{{ route('admin.tables.store') }}" method="POST" class="flex flex-col gap-4" id="createForm" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                  <label for="email-adress-icon" class="block mb-2 text-sm font-medium text-gray-700">Table Name</label>

                  <div class="relative">
                    <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                      <i class="fa-solid fa-utensils"></i>
                    </div>
                    <input type="text" name="name" id="email-adress-icon" class="bg-transparent border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-2 focus:ring-fuchsia-50 focus:border-fuchsia-300 block w-full p-2.5 pl-10 " placeholder="Enter Table's name" value="{{ old('name') }}">
                  </div>
                  @if(isset($errors))
